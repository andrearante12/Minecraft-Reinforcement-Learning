...
class BridgingCFG(BaseCFG):
    ...
    # ── Observation ──────────────────────────────────────────────────────────
    # Proprioception: 6 base (onGround, yaw, pitch, delta_y/x/z)
    #               + 1 inventory count (normalized)
    #               + 1 ray hit flag
    #               + 3 ray hit relative position
    PROPRIOCEPTION_SIZE = 11
    GOAL_DELTA_SIZE     = 3
    INPUT_SIZE          = PROPRIOCEPTION_SIZE + GOAL_DELTA_SIZE + GRID_SIZE  # 214
    ...
    BRIDGING_ACTIONS = [
        ...
        # Sneak (hold/release — same pattern as all other actions)
        ("sneak",             ["sneak 1"],             ["sneak 0"]),             # 8
        # Block placement
        ("place_block",       ["use 1"],               ["use 0"]),              # 9
        # Combined actions (self-contained: sneak included in on/off commands)
        ("sneak_forward",     ["sneak 1", "move 1"],   ["sneak 0", "move 0"]),  # 10
        ("sneak_backward",    ["sneak 1", "move -1"],  ["sneak 0", "move 0"]),  # 11
        ("sneak_place",       ["sneak 1", "use 1"],    ["sneak 0", "use 0"]),   # 12
        # Idle
        ("no_op",             [],                      []),                      # 13
    ]
    ...
    BRIDGE_Z_START = 3     # first Z of the gap
    BRIDGE_Z_END   = 7     # last Z of the gap (inclusive)
    BRIDGE_Y       = 45    # Y level where bridge blocks should go
    BRIDGE_X_MIN   = -1    # X range for valid bridge placement
    BRIDGE_X_MAX   = 1
